...
public abstract class GameController {
	...
	/**
	 * Constructor generates the list of detectives' color
	 * and Jack's hide out.
	 */
	 ...
	 *  of detectives in the previous game
	 * @param previousCrimeScene set of crime scene that Jack has chosen.
	 * @return true if jack reaches his hide out.
	 */
	private boolean runNight(int night,
			final Set<String> lastLocation, final Set<String> previousCrimeScene) {
		Set<String> possibleCrimeScene = gb.RED_CIRCLES;
		possibleCrimeScene.removeAll(previousCrimeScene);
		
		Night currentNight = new NightConsole(night,
				lastLocation, possibleCrimeScene);
		Detectives detectives = currentNight.getDetectives();
		Jack jack = new Jack(currentNight.getCoachAlley(),
				currentNight.getCrimeScene());
		nightController = new NightController(currentNight, jack,
				detectives, hideOut);
		boolean jackReachesHideOut = nightController.run();
		return jackReachesHideOut;
	}
	/**
	 * Generate randomly the head of investigation in each night.
	 * Head of investigation can place the police tokens in the beginning.
	 */
	private void generateHeadofInvestigation() {
		...
		System.out.println(dColor + " Detective is head of"
				+ " investigation in this night!");
	}
	/**
	 * Ask user to choose hideout from the entire game.
	 * @return Jack's hideout.
	 */
	protected abstract String hideOutGenerator();
	/**
