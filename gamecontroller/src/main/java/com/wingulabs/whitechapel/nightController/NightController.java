...
public class NightController {
     ...
     *            Jack's hide out.
     */
    public NightController(final Night night, final Jack jack, final Detectives detectives, final String hideOut) {
        super();
        this.night = night;
        this.jack = jack;
    ...
    public NightController(final Jack jack, final Detectives detectives, final String hideOut) {
        super();
        this.jack = jack;
        this.detectives = detectives;
        this.hideOut = hideOut;
        this.jackController = new JackConsoleController(gb, jack, night);
        this.detectiveController = new DetectiveConsoleController(gb, detectives);
        // this.detectiveController = new
        // DetectiveEngineController(gb,detectives, jack.getCurrentLocation());
    }
    ...
    public final boolean simpleRun() {
        ...
        while (turn < 15) {
            ...
            if (turn == 15) {
                System.out.println("Jack cannot reach his Hide Out in the night" + ". Game Over!");
                return false;
            }
            List<String> detectivesUpdateLocation = detectiveController.detectivesMove();
            for (int i = 0; i < detectives.getDetectives().length; i++) {
                detectives.getDetectives()[i].setLocation(detectivesUpdateLocation.get(i));
            }
    ...
    public final boolean run() {
        ...
        while (night.getCurrentTurn() < 15) {
            ...
            jack.setCurrentLocation(jackMove);
            if (jack.getCurrentLocation().equals(hideOut)) {
                System.out.println("Jack got to his hide out, night " + (int) (night.getCurrentNight() + 1)
                        + " will start soon...");
                return true;
            }
            if (turn == 15) {
