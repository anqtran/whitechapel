...
/**
 * Contains all the utility methods for the detectives engine which controls detectives move.
 * @author ant
 *
 */
public class DetectivesEngineUtility {
    ...
    /**
     * Get distance between two entities in the map.
     * @param origin stating position
    ...
    public int getDistance(String origin, String destination) throws IOException {
        ...
        try (BufferedReader rdr = new BufferedReader(
                new InputStreamReader(getClass().getClassLoader().getResource(SHORTEST_SQUARE_PATH).openStream()))) {
            String line = null;
            while ((line = rdr.readLine()) != null) {
                ...
                if (line.charAt(0) == 'S' && line.substring(19, line.length()).equals(origin)) {
                    ...
                    while (line.startsWith("D")) {
                        ...
                        if (distanceParts[0].substring(distanceParts[0].indexOf('S'), distanceParts[0].length())
                                .equals(destination)) {
    ...
    /**
     * Get the previous vertex in the path of a specific destination.
     * This method uses the shortest square path to figure out the vertex.
     * @param source the starting position
     * @param destination the ending positon.
     * @return the vertex before the destination.
     * @throws IOException
     */
    ...
    /**
     * 
     * @param dts array of detectives
     * @param priorityVertex vertex should be checked
     * @return the first element is the smallest distance, the second is te i
     * @throws IOException throw when cannot find file 
     */
    ...
    public LinkedList<String> getDetectivePathHelper(String source, String destination, LinkedList<String> path)
            throws IOException {
